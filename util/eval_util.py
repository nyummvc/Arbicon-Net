...
def compute_metric(metric,model,dataset,dataloader,batch_tnf,batch_size,two_stage=True,do_aff=False,do_tps=False,args=None):
    ...
    for i, batch in enumerate(dataloader):
        ...
        if metric=='inlier_count':
            stats = inlier_count(batch,batch_start_idx,theta_aff,theta_tps,theta_aff_tps,corr_aff,corr_aff_tps,model.GridDeformation2,
                                 stats,args)
        elif metric_fun is not None:
            stats = metric_fun(batch,batch_start_idx,theta_aff,theta_tps,theta_aff_tps,model.GridDeformation2,stats,args)
...
def point_dist_metric(batch,batch_start_idx,theta_aff,theta_tps,theta_aff_tps,model_tps,stats,args,use_cuda=True):
...
def inlier_count(batch,batch_start_idx,theta_tps,theta_aff_tps,corr_aff,corr_aff_tps,model_tps,stats,args,use_cuda=True):
    inliersComposed = TwoStageWeakInlierCount(use_cuda=torch.cuda.is_available(),dilation_filter=0,normalize_inlier_count=True)
    inliers_comp = inliersComposed(matches=corr_aff,theta_aff=theta_aff,theta_aff_tps=theta_aff_tps,model_tps=model_tps)
    current_batch_size=batch['source_im_size'].size(0)
    indices = range(batch_start_idx,batch_start_idx+current_batch_size)
...
def pascal_parts_metrics(batch,batch_start_idx,theta_aff,theta_tps,theta_aff_tps,model_tps,stats,args,use_cuda=True):
    do_aff = theta_aff is not None
    do_tps = theta_tps is not None
    do_aff_tps = theta_aff_tps is not None
    ...
    for b in range(batch_size):
        ...
        if batch['keypoint_A'][b].size!=0:
            ...
            stats = pck_metric(batch_b,
                               ...
                               theta_aff_tps[b,:].unsqueeze(0) if do_aff_tps else None,model_tps,
                               stats,args,use_cuda)
        else:
            if do_aff:
                stats['aff']['pck'][idx] = -1
        ...
        grid_aff,grid_tps,grid_aff_tps=theta_to_sampling_grid(h_tgt,w_tgt,
                                                              ...
                                                              theta_aff_tps[b,:] if do_aff_tps else None,model_tps,
                                                              use_cuda=use_cuda,
                                                              tps_reg_factor=args.tps_reg_factor)
...
def area_metrics(batch,batch_start_idx,theta_aff,theta_tps,theta_aff_tps,model_tps,stats,args,use_cuda=True):
    do_aff = theta_aff is not None
    do_tps = theta_tps is not None
    do_aff_tps = theta_aff_tps is not None
