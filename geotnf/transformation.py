...
class ComposedGeometricTnf(object):
    ...
    def __call__(self, image_batch, theta_aff, theta_aff_tps, model_tps, use_cuda=True):
...
class GeometricTnf(object):
    ...
    def __init__(self, geometric_model='affine', tps_grid_size=3, tps_reg_factor=0, out_h=240, out_w=240, offset_factor=None, use_cuda=True):
        ...
        elif geometric_model=='def':
            self.gridGen = DefGridGen(out_h, out_w, use_cuda=use_cuda)
    ...
    def __call__(self, image_batch, theta_batch=None, out_h=None, out_w=None, return_warped_image=True, return_sampling_grid=False, padding_factor=1.0, crop_factor=1.0,model=None):
        ...
        if self.geometric_model=='def':
            sampling_grid = gridGen(theta_batch,model=model)
        else:
...
class DefGridGen(Module):
    def __init__(self, out_h=240, out_w=240, use_cuda=True):
        super(DefGridGen, self).__init__()
        self.out_h, self.out_w = out_h, out_w
        self.use_cuda = use_cuda
        ...
        self.grid_X,self.grid_Y = np.meshgrid(np.linspace(-1,1,out_w),np.linspace(-1,1,out_h))
        # grid_X,grid_Y: size [1,H,W,1,1]
        self.grid_X = torch.FloatTensor(self.grid_X).unsqueeze(0)
        self.grid_Y = torch.FloatTensor(self.grid_Y).unsqueeze(0)
        self.grid_X = Variable(self.grid_X,requires_grad=False)
        self.grid_Y = Variable(self.grid_Y,requires_grad=False)
        if use_cuda:
            self.grid_X = self.grid_X.cuda()
            self.grid_Y = self.grid_Y.cuda()
    def forward(self, theta, model):
        b = theta.size(0)
        point = torch.cat([self.grid_X.unsqueeze(-1), self.grid_Y.unsqueeze(-1)],dim=-1)
        point = point.view(1,-1,2).permute(0,2,1).contiguous().repeat(b,1,1)
        offsets = model(theta, point)
        point = point + offsets
        point = point.permute(0,2,1).contiguous().view(b,self.out_h,self.out_w,2)
        return point
