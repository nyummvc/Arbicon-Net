...
from model.layers import Conv4d, BatchNorm4d
...
class FRConv(torch.nn.Module):
    def __init__(self, use_cuda=True, kernel_sizes=[3,3,3], channels=[10,10,1], symmetric_mode=True):
        ...
        for i in range(num_layers):
            ...
            nn_modules.append(BatchNorm4d(ch_out))
...
class FeatureRegression4D(nn.Module):
    def __init__(self, output_dim=6, use_cuda=True, kernel_sizes=[3,3,3], channels=[10,10,1], symmetric_mode=True, feature_size=15):
        super(FeatureRegression4D, self).__init__()

        self.use_cuda = use_cuda
        self.FRConv = FRConv(use_cuda=self.use_cuda,
                                             kernel_sizes=kernel_sizes,
                                             channels=channels)
        self.linear = nn.Linear(feature_size**4,output_dim)
        if use_cuda:
            self.linear.cuda()

    def forward(self, corr4d):
        corr4d = MutualMatching(corr4d)
        corr4d = self.FRConv(corr4d)
        corr4d = MutualMatching(corr4d)
        corr4d = corr4d.view(corr4d.size(0),-1)
        x = self.linear(corr4d)
        return x

class GridDeformation(nn.Module):
    def __init__(self, input_dim=256, use_cuda=True):
        super(GridDeformation, self).__init__()
        self.use_cuda = use_cuda
        self.mlp = nn.Sequential(
            nn.Conv1d(input_dim+2, 256, kernel_size=1),
            nn.BatchNorm1d(256),
            nn.Softplus(),
...
class CNNGeometric(nn.Module):
    def __init__(self, output_dim=256,
                 ...
                 train_fe=False,use_cuda=True,consis=False):
        ...
        if output_dim < 256:
            self.FeatureCorrelation = FeatureCorrelation(shape='3D',normalization=True)
            self.FeatureRegression = FeatureRegression3D(output_dim,use_cuda=self.use_cuda,
            ...
            self.FeatureRegression = FeatureRegression4D(output_dim,use_cuda=use_cuda,
                                                    feature_size=fr_feature_size_four,kernel_sizes=fr_kernel_sizes_four,
                                                    channels=fr_channels_four)
            self.GridDeformation = GridDeformation(output_dim,use_cuda=self.use_cuda)
    ...
    def forward(self, tnf_batch):
        ...
        # regression to tnf parameters theta
            ...
            corr_ab = self.FeatureCorrelation(feature_A,feature_B)
            theta_ab = self.FeatureRegression(corr_ab)
            corr_ba = self.FeatureCorrelation(feature_B,feature_A)
            theta_ba = self.FeatureRegression(corr_ba)
            corr_aa = self.FeatureCorrelation(feature_A,feature_A)
            theta_aa = self.FeatureRegression(corr_aa)
            corr_bb = self.FeatureCorrelation(feature_B,feature_B)
            theta_bb = self.FeatureRegression(corr_bb)
            return theta_ab, theta_ba, theta_aa, theta_bb
...
class TwoStageCNNGeometric(CNNGeometric):
    ...
    def forward(self, batch, f_src=None, f_tgt=None, use_theta_GT_aff=False):
        ...
        # feature correlation
        correlation_2 = self.FeatureCorrelation2(f_src_wrp,f_tgt)
        # regression to tnf parameters theta
        theta_2 = self.FeatureRegression2(correlation_2)

        if self.return_correlation:
            correlation_1_out = correlation_1
            correlation_2_out = featureL2Norm(F.relu(correlation_2.squeeze(1).transpose(1,2).contiguous().view(b,h*w,h,w)))
            return (theta_1,theta_2,correlation_1_out,correlation_2_out)
        else:
