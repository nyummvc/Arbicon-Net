...
if args.model!='':
    ...
    for name, param in model.GridDeformation2.state_dict().items():
        model.GridDeformation2.state_dict()[name].copy_(checkpoint['state_dict']['GridDeformation2.' + name])
...
if args.model_tps!='':
    ...
    for name, param in model.GridDeformation2.state_dict().items():
        model.GridDeformation2.state_dict()[name].copy_(checkpoint_tps['state_dict']['GridDeformation.' + name])
...
for name,param in model.GridDeformation2.named_parameters():
    param.requires_grad = args.train_gd
    if args.train_fr and name.find('bn')!=-1:
        param.requires_grad = args.train_bn
for name,param in model.GridDeformation2.named_parameters():
    print(name.ljust(30),param.requires_grad)
...
def inlier_score_function(theta_aff,theta_aff_tps,corr_aff,corr_aff_tps,model_tps,minimize_outliers=False):
    inliers_comp = inliersComposed(matches=corr_aff,
                                                 theta_aff=theta_aff,
                                                 theta_aff_tps=theta_aff_tps,model_tps=model_tps)
...
def loss_fun(batch,model):
    ...
    inlier_score_pos = inlier_score_function(theta_aff,
                                             theta_aff_tps,
                                             corr_aff,
                                             corr_aff_tps,model_tps=model.GridDeformation2)
...
stats=compute_metric(metric,model,dataset_eval,dataloader_eval,batch_tnf,2,two_stage,do_aff,do_tps,args)
...
for epoch in range(1, args.num_epochs+1):
    if args.update_bn_buffers==False:
        ...
        model.train()
    train_loss[epoch-1] = process_epoch('train',epoch,model,loss_fun,optimizer,dataloader,batch_tnf,log_interval=50)
    model.eval()    
    stats=compute_metric(metric,model,dataset_eval,dataloader_eval,batch_tnf,2,two_stage,do_aff,do_tps,args)
    eval_value=np.mean(stats['aff_tps'][metric][eval_idx])
    print(eval_value)
